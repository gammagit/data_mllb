...
#subslist <- c(1:23) # All subjects
#subslist <- c(3,4,5,10,11,12,20,23,24) # Subject with optimal difficult behaviour
nsubs <- length(subslist)
pdf(file=paste("results_mlgono_exp5.pdf", sep=""))
vector_coef_intr_e <- NULL
vector_coef_intr_m <- NULL
...
#    logit_m <- glm(action[condition==3 & (coherence==0.72|coherence==0.28)] ~
#                   time[condition==3 & (coherence==0.72|coherence==0.28)] +
#                   evidence[condition==3 & (coherence==0.72|coherence==0.28)],
#                   data=arg_data,
#                   family="binomial")
#    logit_m <- glm(action[condition==3 & (coherence==0.53|coherence==0.03)] ~
#                   time[condition==3 & (coherence==0.53|coherence==0.03)] +
#                   evidence[condition==3 & (coherence==0.53|coherence==0.03)],
#                   data=arg_data,
#                   family="binomial")
...
#                       "ci_e" = confint_e, "ci_d" = confint_d, "ci_m" = confint_m)
    returnlist <- list("easy" = logit_e, "mix" = logit_m,
                       "ci_e" = confint_e, "ci_m" = confint_m)
    return(returnlist)
}
...
for (ix in subslist) {
    ...
    subdata <- bounds_getdata(paste("gono_sub_", ix, ".csv", sep=""), ix)
    alldata <- rbind(alldata, subdata)
    print(ix)

    logit_fits <- bounds_fit_logit(subdata)

    coef_intr_e <- logit_fits$easy$coefficients[1]
    ...
    attributes(coef_intr_e) <- NULL # strip attributes
...
#    attributes(coef_evid_d) <- NULL # strip attributes
    ...
    coef_intr_m <- logit_fits$mix$coefficients[1]
    ...
    attributes(coef_intr_m) <- NULL # strip attributes
    attributes(coef_time_m) <- NULL # strip attributes
    attributes(coef_evid_m) <- NULL # strip attributes

    vector_coef_intr_e <- c(vector_coef_intr_e, coef_intr_e)
    vector_coef_intr_m <- c(vector_coef_intr_m, coef_intr_m)
...
intercept_e <- (-1) * vector_coef_intr_e / vector_coef_evid_e
intercept_m <- (-1) * vector_coef_intr_m / vector_coef_evid_m
...
em_test <- t.test(slopes_e, slopes_m, paired=TRUE)
print(em_test)
...
### Partial-pooling (multilevel) analysis

mlogit_e <- glmer(action ~ time + evidence + (1 + time + evidence | individual),
                 data=alldata[alldata$condition==1,],
                 family=binomial(link="logit"))
mlogit_d <- glmer(action ~ time + evidence + (1 + time + evidence | individual),
                 data=alldata[alldata$condition==2,],
                 family=binomial(link="logit"))
mlogit_m <- glmer(action ~ time + evidence + (1 + time + evidence | individual),
                 data=alldata[alldata$condition==3,],
                 family=binomial(link="logit"))
#display(mlogit_e)

ml_coef_e <- coef(mlogit_e)
ml_coef_d <- coef(mlogit_d)
ml_coef_m <- coef(mlogit_m)

ml_intercept_e <- (-1) * ml_coef_e$individual[1] / ml_coef_e$individual[3]
ml_intercept_m <- (-1) * ml_coef_m$individual[1] / ml_coef_m$individual[3]
ml_slopes_e <- (-1) * ml_coef_e$individual$time / ml_coef_e$individual$evidence
ml_slopes_m <- (-1) * ml_coef_m$individual$time / ml_coef_m$individual$evidence

bounds_plot_slopes(1, ml_slopes_e, ml_slopes_m)
...
write.csv(mc_data, file="mcdata_exp5.csv", row.names=FALSE)
