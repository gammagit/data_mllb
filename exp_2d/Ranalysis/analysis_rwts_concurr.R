...
pdf(file=paste("results_cwts_exp3.pdf", sep=""))
alldata <- NULL
all_meanwts <- NULL
all_sewts <- NULL
all_se_wi <- NULL
...
bounds_plot_wts <- function(arg_stimixs, arg_meanwts, arg_sewts, arg_se_wi)
{
...
#           c("easy", "mixed", "diff"),
           ...
           lty=1, lwd=2,
           col=clfg[1:2])
    title(main = "Stimuli weights, Experiment 3", cex=1.5)
}
...
### Stim wt analysis
    ...
    wtlogit_e <- glmer(decision ~ s1+s2+s3+s4+s5 + (1 + s1+s2+s3+s4+s5 | individual),
                       data=alldata[alldata$condition==1,],
                       family=binomial(link="logit"))
    wtlogit_m <- glmer(decision ~ s1+s2+s3+s4+s5 + (1 + s1+s2+s3+s4+s5 | individual),
                       data=alldata[alldata$condition==3,],
                       family=binomial(link="logit"))
    iwtlogit_e <- glmer(decision ~ r1+r2+r3+r4+r5 + (1 + r1+r2+r3+r4+r5 | individual),
                       data=alldata[alldata$condition==1,],
                       family=binomial(link="logit"))
    iwtlogit_m <- glmer(decision ~ r1+r2+r3+r4+r5 + (1 + r1+r2+r3+r4+r5 | individual),
                       data=alldata[alldata$condition==3,],
                       family=binomial(link="logit"))

    for (ii in seq(1,5)) {
        ### Get average effect of stimuli ii across participants
        meanwt_e <- fixef(wtlogit_e)[ii+1] # Fixed effect for regression slope
        sewt_e <- se.fixef(wtlogit_e)[ii+1] # Std Error for regression slope
        se_wi_e <- attributes(VarCorr(wtlogit_e)$individual)$stddev[ii+1] # Within-individual variance
        meanwt_m <- fixef(wtlogit_m)[ii+1]
        sewt_m <- se.fixef(wtlogit_m)[ii+1]
        se_wi_m <- attributes(VarCorr(wtlogit_m)$individual)$stddev[ii+1]
        ...
        ## Aligned at decision
        imeanwt_e <- fixef(iwtlogit_e)[ii+1] # Fixed effect for regression slope
        isewt_e <- se.fixef(iwtlogit_e)[ii+1] # Std Error for regression slope
        ise_wi_e <- attributes(VarCorr(iwtlogit_e)$individual)$stddev[ii+1] # Within-individual variance
        imeanwt_m <- fixef(iwtlogit_m)[ii+1]
        isewt_m <- se.fixef(iwtlogit_m)[ii+1]
        ise_wi_m <- attributes(VarCorr(iwtlogit_m)$individual)$stddev[ii+1]
...
bounds_plot_wts(seq(1,nrow(all_meanwts)), all_meanwts, all_sewts, all_se_wi)
bounds_plot_wts(seq(1,nrow(iall_meanwts)), iall_meanwts, iall_sewts, iall_se_wi)
