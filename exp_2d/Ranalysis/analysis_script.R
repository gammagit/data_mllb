...
subslist <- c(1:15,19:24) # All subjects
...
pdf(file=paste("results_linreg_exp3.pdf", sep=""))
...
#                       "le" = lmfit_e, "ld" = lmfit_d, "lm" = lmfit_m)
    returnlist <- list("re" = rlmfit_e, "rm" = rlmfit_m,
                       "le" = lmfit_e, "lm" = lmfit_m)
    return(returnlist)
}
...
bounds_dec_stats <- function(arg_data)
{
    ncorrect_easy = sum(1*(((arg_data$decision[arg_data$condition=='easy'] == 1) &
                            (arg_data$coherence[arg_data$condition=='easy'] == 0.72)) |
                           ((arg_data$decision[arg_data$condition=='easy'] == 0) &
                            (arg_data$coherence[arg_data$condition=='easy'] == 0.28))))
    ntotal_easy = sum(1*(arg_data$condition=='easy'))
    ncorrect_mix = sum(1*(((arg_data$decision[arg_data$condition=='mix'] == 1) &
                           (arg_data$coherence[arg_data$condition=='mix'] == 0.72)) |
                          ((arg_data$decision[arg_data$condition=='mix'] == 0) &
                           (arg_data$coherence[arg_data$condition=='mix'] == 0.28))))
    ntotal_mix = sum(1*(arg_data$coherence[arg_data$condition=='mix'] == 0.72 |
                        arg_data$coherence[arg_data$condition=='mix'] == 0.28))
    nhard_mix <- sum(arg_data$coherence[arg_data$condition=='mix'] == 0.50 | 
                     arg_data$coherence[arg_data$condition=='mix'] == 0.50)
...
bounds_btest <- function(arg_data)
{
    dec_stats <- bounds_dec_stats(arg_data)
    ...
    if(btest_easy$p.value > 0.05) {
        print(paste("Subject ", ix,
                    " failed Binomial test in Easy condition",
                    sep=""))
    }
    if(btest_mix$p.value > 0.05) {
        print(paste("Subject ", ix,
                    " failed Binomial test in Mix condition",
                    sep=""))
    }
}
...
bounds_getRT <- function(arg_data)
{
    ...
    if (length(arg_data$condition=='diff') > 0) {
    ...
    rt_mixblock_easy.med <- median(arg_data$time[arg_data$condition=='mix'&
                                                 (arg_data$coherence == 0.72 |
                                                  arg_data$coherence == 0.28)])
    rt_mixblock_diff.med <- median(arg_data$time[arg_data$condition=='mix'&
                                                 (arg_data$coherence == 0.50 |
                                                 arg_data$coherence == 0.50)])
    ...
    rt_easyblock_cor.mu <- with(arg_data,
                                mean(time[condition=='easy' &
                                          (coherence == 0.72 & decision == 1) |
                                          (coherence == 0.28 & decision == 0)]))
    rt_easyblock_err.mu <- with(arg_data,
                               mean(time[condition=='easy' &
                                         (coherence == 0.72 & decision == 0) |
                                         (coherence == 0.28 & decision == 1)]))
...
bounds_plot_props <- function(arg_prop_vector_e, arg_prop_vector_m)
{
    plot(arg_prop_vector_e, arg_prop_vector_m, xlab="Proportion Easy",
        ylab="Proportion Mixed", xlim=c(0,0.5), ylim=c(0,0.5), pch=16)
...
#    prop_vector_m <- c(prop_vector_m, norm_prop_dec.mix)
    ...
    # Save all variables
    save(list=ls(), file=paste("results_sub_", ix, ".RData", sep=""))
...
save(list=ls(), file=paste("results_allsubs.RData", sep=""))
