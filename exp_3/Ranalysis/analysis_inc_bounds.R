...
pdf(file=paste("rapid_expt_two", blockno, ".pdf", sep=""))
...
bounds_plotsubdata <- function(arg_data, subid)
{
    ...
    scatterplot(jitter(evidence_ud,1) ~ jitter(time,1) | condition,
                data=arg_data,
                lwd=2,
                boxplots='',
                xlab='number of samples', ylab='evidence',
                legend.coords="topright",
                reg.line=rlm,
                smoother=FALSE,
                xlim=c(0, 25), ylim=c(-5, 10),
#                pch=c(16,4,21),
                pch=c(21,21,21),
                cex=1.5,
                cex.lab=1.5,
    ...
    mtext(paste("Sub ", subid, sep=""), side=3, line=2, adj=1, cex=1.5)

    coplot(jitter(evidence_ud,1) ~ jitter(time,1) | condition,
           data=arg_data,
           panel=function(x,y,...){
                usr<-par("usr")
                rect(usr[1], usr[3], usr[2], usr[4], col="white")
                abline(h=seq(-3,15,by=1),v=seq(0,20,by=1),col='gray97',lwd=0.5)
                abline(h=0, v=0, lty=2, lwd=0.5)
                points(x, y,...)
                abline(rlm(y~x,maxit=50), lty=1, lwd=1.5, col="red")
           },
           col=gray(0.5),
           columns=3,
           xlim=c(0, 15), ylim=c(-3, 10),
           xlab='number of samples', ylab='evidence',
           pch=21,
           cex=1,
           cex.lab=1.5)
...
bounds_plothist <- function(arg_data)
{
    ...
    plot(hist_easy, axes=F, xlab="", ylab="", main="",
         ...
         xlim=c(0,50)*(25/50)-0.5, ylim=c(-2,0)-0.08)
    ...
    plot(hist_mix, axes=F, xlab="", ylab="", main="",
         ...
         xlim=c(0,50)*(25/50)-0.5, ylim=c(0,2)+0.08)
}
...
{
    ncorrect_easy = sum(1*(((arg_data$decision[arg_data$condition=='easy'] == 0) &
                            (arg_data$coherence[arg_data$condition=='easy'] == 0.90)) |
                           ((arg_data$decision[arg_data$condition=='easy'] == 1) &
                            (arg_data$coherence[arg_data$condition=='easy'] == 0.10))))
    ntotal_easy = sum(1*(arg_data$condition=='easy'))
    ncorrect_mix = sum(1*(((arg_data$decision[arg_data$condition=='mix'] == 0) &
                           (arg_data$coherence[arg_data$condition=='mix'] == 0.90)) |
                          ((arg_data$decision[arg_data$condition=='mix'] == 1) &
                           (arg_data$coherence[arg_data$condition=='mix'] == 0.10))))
    ntotal_mix = sum(1*(arg_data$coherence[arg_data$condition=='mix'] == 0.90 |
                        arg_data$coherence[arg_data$condition=='mix'] == 0.10))
    nhard_mix <- sum(arg_data$coherence[arg_data$condition=='mix'] == 0.60 | 
                     arg_data$coherence[arg_data$condition=='mix'] == 0.40)
...
bounds_getRT <- function(arg_data)
{
    ...
    rt_mixblock_easy.med <- median(arg_data$time[arg_data$condition=='mix'&
                                                 (arg_data$coherence == 0.90 |
                                                  arg_data$coherence == 0.10)])
    rt_mixblock_diff.med <- median(arg_data$time[arg_data$condition=='mix'&
                                                 (arg_data$coherence == 0.60 |
                                                 arg_data$coherence == 0.40)])
    ...
    rt_easyblock_cor.mu <- with(arg_data,
                                mean(time[condition=='easy' &
                                          (coherence == 0.90 & decision == 0) |
                                          (coherence == 0.10 & decision == 1)]))
    rt_easyblock_err.mu <- with(arg_data,
                               mean(time[condition=='easy' &
                                         (coherence == 0.90 & decision == 1) |
                                         (coherence == 0.10 & decision == 0)]))
...
bounds_getacc_at_RTs <- function(arg_data, subid)
{
...
for (ix in subslist) {
    ...
    sub_acc_at_RTs <- bounds_getacc_at_RTs(subdata, ix)
...
#    prop_vector_m <- c(prop_vector_m, norm_prop_dec.mix)
    ...
    bounds_plotsubdata(subdata, ix)
#    mtext(paste("Sub ", ix, sep=""), side=3, line=2, adj=1, cex=1.5)
...
# Paired t-test for difference in RTs between easy and mixed
diff_ed <- RT_vector_e.mu - RT_vector_d.mu
sd_diff_ed <- sd(diff_ed)
se_diff_ed <- sd_diff_ed / sqrt(length(diff_ed))
Tval_diff_ed <- mean(diff_ed) / se_diff_ed
Tval_table_diff_ed = qt(0.975, df=(length(diff_ed)-1))
print("")
print(paste("mean(RTs_easy - RTs_diff) = ", mean(diff_ed), sep=""))
print(paste("t-value = ", Tval_diff_ed, sep=""))
print(paste("t-value for p<0.05= ", Tval_table_diff_ed, sep=""))

# Paired t-test for difference in RTs between easy and diff during mixed block
diff_med <- RT_vector_m_e.mu - RT_vector_m_d.mu
sd_diff_med <- sd(diff_med)
se_diff_med <- sd_diff_med / sqrt(length(diff_med))
Tval_diff_med <- mean(diff_med) / se_diff_med
Tval_table_diff_med = qt(0.975, df=(length(diff_med)-1))
print("")
print(paste("mean(RTs_mix_easy - RTs_mix_diff) = ", mean(diff_med), sep=""))
print(paste("t-value = ", Tval_diff_med, sep=""))
print(paste("t-value for p<0.05= ", Tval_table_diff_med, sep=""))
